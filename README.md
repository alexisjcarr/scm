...
`scm` is a small but complete host configuration management system written in Go. I intentionally designed it as a distributed service rather than a one-off script.
...
Work dispatch is agent-pulled and lease-backed. Agents register and heartbeat with the control plane, poll for work when idle, receive an atomically claimed work item plus lease token, reconcile local state, and report progress and terminal status back to the control plane.
...
- stalled applies are detected conservatively from missing heartbeats and expired work leases, but the system does not yet automatically reclaim and retry stalled work
